"use client";
import {useState} from "react";import {useRouter,useSearchParams} from "next/navigation";import {Globe2,Loader2} from "lucide-react";import {supabaseBrowser} from "@/lib/supabase/client";
const field="w-full rounded-lg border border-[#dfe4e0] px-3 py-2.5 text-sm outline-none focus:border-[#176b4d]";
export function LoginForm({brand}:{brand:string}){const router=useRouter();const params=useSearchParams();const [email,setEmail]=useState("");const [password,setPassword]=useState("");const [error,setError]=useState("");const [busy,setBusy]=useState(false);
